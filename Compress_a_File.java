 ...
 * Group responsible for this program: Sarah King, Kha Hoang
 * Assignment Name: Programming Project
 * Description: This program takes in a file and compress it using Huffman coding.
 */
...
public class Compress_a_File {
	public static void main(String[] args) throws IOException {
		...
		DataInputStream sourceFileInputStream = new DataInputStream(
				new BufferedInputStream(new FileInputStream(sourceFile)));
		int size = sourceFileInputStream.available();
		byte[] b = new byte[size];
		sourceFileInputStream.read(b);
		sourceFileInputStream.close();
		String text = new String(b);

		int[] freqCounts = getCharacterFrequency(text);
		Tree tree = getHuffmanTree(freqCounts);
	...
	public static Tree getHuffmanTree(int[] freqCounts) {
		// Create a heap to hold trees
		Heap<Tree> heap = new Heap<Tree>(); // Defined in Listing 24.10
		for (int i = 0; i < freqCounts.length; i++) {
			if (freqCounts[i] > 0)
				heap.add(new Tree(freqCounts[i], (char) i)); // A leaf node tree
		}
	...
	public static int[] getCharacterFrequency(String text) {
		int[] freqCounts = new int[256]; // 256 ASCII characters
		for (int i = 0; i < text.length(); i++) {
			freqCounts[(int) text.charAt(i)]++; // Count the character in text
		}
		return freqCounts;
	}
